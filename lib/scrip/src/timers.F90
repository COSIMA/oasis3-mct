...
!-----------------------------------------------------------------------
      ...
      integer (kind=int_kind), parameter ::  max_timers = 99  ! max number of timers allowed

      integer (kind=int_kind), save :: cycles_max       ! max value of clock allowed by system

      integer (kind=int_kind), dimension(max_timers), save :: cycles1,  &       ! cycle number at start for each timer
                                                              cycles2           ! cycle number at stop  for each timer

      real (kind=real_kind), save :: clock_rate       ! clock_rate in seconds for each cycle

      real (kind=real_kind), dimension(max_timers), save :: cputime          ! accumulated cpu time in each timer

      character (len=8), dimension(max_timers), save :: status           ! timer status string

      character (len=char_len), dimension(max_timers), save :: name
...
!-----------------------------------------------------------------------
      ...
      integer (kind=int_kind), intent(in) ::  timer            ! timer number
...
!-----------------------------------------------------------------------
      ...
      real (kind=real_kind) :: timer_get   ! accumulated cputime in given timer
...
!-----------------------------------------------------------------------
      ...
      if (status(timer) .eq. 'stopped') THEN
        if (nlogprt .GE. 2) THEN
          if (trim(name(timer)) .eq. 'noname') then
             write(nulou,"(' Elapsed time for timer',i3,':',1p,e16.8)") timer,cputime(timer)
          else
             write(nulou, "(' Elapsed time for timer ',A24,':',1x,f10.4)") name(timer),cputime(timer)
          end if
        end if
      else
        call timer_stop(timer)
...
!***********************************************************************
      ...
      subroutine timer_start(timer,string)
...
!-----------------------------------------------------------------------
      ...
      if (status(timer) .eq. 'running') then
        ...
        if (cycles2(timer) .ge. cycles1(timer)) then
          cputime(timer) = cputime(timer) + clock_rate*(cycles2(timer) - cycles1(timer))
        else
          cputime(timer) = cputime(timer) + clock_rate*(cycles2(timer) - cycles1(timer) + cycles_max)
        endif
