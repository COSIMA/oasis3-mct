...
! !USES:
   ...
   use mod_prism_method ,only: oasis_init_comp_proto     => prism_method_init
   use mod_prism_method ,only: oasis_terminate_proto     => prism_method_terminate
   use mod_prism_method ,only: oasis_get_localcomm_proto => prism_method_getlocalcomm
   use mod_prism_method ,only: oasis_enddef_proto        => prism_method_enddef

   use mod_prism_part   ,only: oasis_def_partition_proto => prism_part_def

   use mod_prism_var    ,only: oasis_def_var_proto       => prism_var_def

   use mod_prism_getput_interface ,only: oasis_get_proto => prism_get_proto
   use mod_prism_getput_interface ,only: oasis_put_proto => prism_put_proto
   ...
   use mod_prism_sys    ,only: oasis_abort_proto         => prism_sys_abort
