...
!                    algorithm)
!
! Modified by T.Craig for Oasis, May 2017 based on the public version in
! CESM1.2.
! 
!-----------------------------------------------------------------------
...
!-----------------------------------------------------------------------
   ...
   use mod_oasis_kinds, only: r8 => ip_r8_p, i8 => ip_i4_p
#else
   use mod_oasis_kinds, only: r8 => ip_r8_p, i8 => ip_i8_p
#endif
...
!-----------------------------------------------------------------------
   public :: &
      oasis_reprosum_setopts,        &! set runtime options
      oasis_reprosum_calc,           &! calculate distributed sum
      oasis_reprosum_tolExceeded      ! utility function to check relative 
...
!-----------------------------------------------------------------------
   ...
   subroutine oasis_reprosum_setopts(repro_sum_use_ddpdd_in,    &
...
!---------------------------Local Workspace-----------------------------
      ...
      character(len=*),parameter :: subname = '(oasis_reprosum_setopts)'
!-----------------------------------------------------------------------
      ...
      if (.not. firstcall) then
         write(llogunit,*) subname,estr,' can only be called once'
      ...
      if ( present(repro_sum_rel_diff_max_in) ) then
         oasis_reprosum_reldiffmax = repro_sum_rel_diff_max_in
      endif
      if ( present(repro_sum_recompute_in) ) then
         oasis_reprosum_recompute = repro_sum_recompute_in
      endif
      if (master) then
         ...
         if (oasis_reprosum_reldiffmax >= 0._r8) then
            write(llogunit,*) subname, &
              ' with a maximum relative error tolerance of ', &
              oasis_reprosum_reldiffmax
            if (oasis_reprosum_recompute) then
               write(llogunit,*) subname, &
   ...
   end subroutine oasis_reprosum_setopts
...
! returned.
!
! If requested (by setting oasis_reprosum_reldiffmax >= 0.0 and passing in
...
!----------------------------------------------------------------------
   subroutine oasis_reprosum_calc (arr, arr_gsum, nsummands, dsummands,     &
                                   nflds, ddpdd_sum,                        &
...
! Local workspace
      ...
      logical :: use_ddpdd_sum           ! flag indicating whether to
                                         !  use oasis_reprosum_ddpdd or not
...
#endif  
      character(len=*),parameter :: subname = '(oasis_reprosum_calc)'

!-----------------------------------------------------------------------

! check whether should use oasis_reprosum_ddpdd algorithm
...
! set MPI communicator
      ...
      if ( use_ddpdd_sum ) then
         ...
         if (detailed_timing) call oasis_timer_start('oasis_reprosum_ddpdd')

         call oasis_reprosum_ddpdd(arr, arr_gsum, nsummands, dsummands, &
                              nflds, mpi_comm)
         repro_sum_fast = 1

         if (detailed_timing) call oasis_timer_stop('oasis_reprosum_ddpdd')

      else

         if (detailed_timing) call oasis_timer_start('oasis_reprosum_int')
...
! performance for loopb in oasis_reprosum_int
...
! algorithm
               if (detailed_timing) call oasis_timer_start("repro_sum_allr_max")
               ...
               if (detailed_timing) call oasis_timer_stop("repro_sum_allr_max")
...
! then over threads and tasks
               ...
               if (arr_max_shift < 2) then
                  write(nulprt,*) subname,estr,' number of summands too large for fixed precision algorithm'
                  call oasis_abort(file=__FILE__,line=__LINE__)
               endif
...
! calculate sum
               ...
               call oasis_reprosum_int(arr, arr_gsum, nsummands, dsummands, &
...
!$omp private(ithread, ifld, isum, arr_exp, arr_exp_tlmin, arr_exp_tlmax)
            do ithread=1,omp_nthreads
               if (detailed_timing) call oasis_timer_start('repro_sum_loopa')
               do ifld=1,nflds
               ...
               if (detailed_timing) call oasis_timer_stop('repro_sum_loopa')
            end do
            ...
            if (detailed_timing) call oasis_timer_start("repro_sum_allr_minmax")
            call mpi_allreduce (arr_lextremes, arr_gextremes, 2*(nflds+1), &
                                MPI_INTEGER, MPI_MIN, mpi_comm, ierr)
            if (detailed_timing) call oasis_timer_stop("repro_sum_allr_minmax")
...
! calculate sum
            validate = .false.
            call oasis_reprosum_int(arr, arr_gsum, nsummands, dsummands, nflds, &
         ...
         if (detailed_timing) call oasis_timer_stop('oasis_reprosum_int')

      endif

! compare fixed and floating point results
      if ( present(rel_diff) ) then
         if (oasis_reprosum_reldiffmax >= 0.0_r8) then
            ...
            if (detailed_timing) call oasis_timer_start('nonrepro_sum')
! record statistic
            nonrepro_sum = 1
...
!$omp private(ifld, isum)
            ...
            if (detailed_timing) call oasis_timer_stop('nonrepro_sum')
...
! return statistics
   ...
   end subroutine oasis_reprosum_calc
...
!----------------------------------------------------------------------
   ...
   subroutine oasis_reprosum_int (arr, arr_gsum, nsummands, dsummands, nflds, &
                                  arr_max_shift, arr_gmax_exp, max_levels,    &
                                  max_level, validate, recompute,             &
                                  omp_nthreads, mpi_comm                      )
...
! Local workspace
      ...
      character(len=*),parameter :: subname = '(oasis_reprosum_int)'
...
!$omp         arr_shift, ilevel, i8_arr_level, arr_remainder, RX_8, IX_8)
      do ithread=1,omp_nthreads
       if (detailed_timing) call oasis_timer_start('repro_sum_loopb')
       do ifld=1,nflds
          ioffset = offset(ifld)
...
! over threads and processes do not overflow for ilevel==1.
          ...
          enddo
       enddo
       if (detailed_timing) call oasis_timer_stop('repro_sum_loopb')
      enddo
...
! sum integer vector element-wise
     ...
     ! Workaround for when i8 is not supported.
      if (detailed_timing) call oasis_timer_start("repro_sum_allr_i4")
      call mpi_allreduce (i8_arr_lsum_level, i8_arr_gsum_level, & 
                          veclth, MPI_INTEGER, MPI_SUM, mpi_comm, ierr)
      if (detailed_timing) call oasis_timer_stop("repro_sum_allr_i4")
#else
      if (detailed_timing) call oasis_timer_start("repro_sum_allr_i8")
      call mpi_allreduce (i8_arr_lsum_level, i8_arr_gsum_level, & 
                          veclth, MPI_INTEGER8, MPI_SUM, mpi_comm, ierr)
      if (detailed_timing) call oasis_timer_stop("repro_sum_allr_i8")
#endif
...
! subsequent levels contribute arr_max_shift digits.
   ...
   end subroutine oasis_reprosum_int
...
!----------------------------------------------------------------------
   ...
   logical function oasis_reprosum_tolExceeded (name, nflds, master, &
                                                logunit, rel_diff    )
!----------------------------------------------------------------------
...
! Local workspace
      ...
      character(len=*),parameter :: subname = '(oasis_reprosum_tolExceeded)'

!-----------------------------------------------------------------------

      oasis_reprosum_tolExceeded = .false.
      if (oasis_reprosum_reldiffmax < 0.0_r8) return
      ...
      max_abs_diff = 0.0_r8
      do ifld=1,nflds
         if (rel_diff(1,ifld) > oasis_reprosum_reldiffmax) then
      ...
      if (exceeds_limit > 0) then
         ...
         oasis_reprosum_tolExceeded = .true.
       endif


   end function oasis_reprosum_tolExceeded
...
!----------------------------------------------------------------------
   ...
   subroutine oasis_reprosum_ddpdd (arr, arr_gsum, nsummands, dsummands,  &
                                    nflds, mpi_comm                       )
!----------------------------------------------------------------------
...
! Local workspace
      ...
      character(len=*),parameter :: subname = '(oasis_reprosum_ddpdd)'
...
!-----------------------------------------------------------------------
   ...
   end subroutine oasis_reprosum_ddpdd
...
! Local workspace
      ...
      real(r8) e, t1, t2
      integer i
      character(len=*),parameter :: subname = '(oasis_reprosum_mod:DDPDD)'
...
! Local workspace
    ...
    character(len=*),parameter :: subname = '(oasis_reprosum_mod:split_indices)'
