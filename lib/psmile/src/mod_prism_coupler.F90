     ...
     do nv1 = 1,nallvar(compid)
        ...
        if (flag /= PRISM_NotDef) then
           ...
           if (flag /= PRISM_In .and. flag /= PRISM_Out) then
              ...
              call prism_sys_abort(compid,subname,'ERROR flag problems')
           endif
              ...
              if (otfldi == myfldi) then
                 ...
                 if (namfldops(nn) == ip_exported .or. namfldops(nn) == ip_expout) then
                    if (nm == compid) then
                       ...
                       call prism_sys_abort(compid,subname,' ERROR send recv pair on same model')
                    endif
                    if (flag == PRISM_Out .and. allops(nv,nm) /= PRISM_In) then
                       write(nulprt,*) subname,' ERROR send recv pair both Out ', &
                          trim(myfld),' ',trim(otfld)
                       call prism_sys_abort(compid,subname,' ERROR send recv pair both Out')
                    endif
                    if (flag == PRISM_In .and. allops(nv,nm) /= PRISM_Out) then
                       write(nulprt,*) subname,' ERROR send recv pair both In ', &
                          trim(myfld),' ',trim(otfld)
                       call prism_sys_abort(compid,subname,' ERROR send recv pair both In')
                    endif
                 endif
                 ...
                 if (namfldops(nn) == ip_input .or. namfldops(nn) == ip_output) then
                    if (trim(myfld) /= trim(otfld)) then
                       write(nulprt,*) subname,' ERROR namcouple field names to not match for in/out', &
                          trim(myfld),' ',trim(otfld)
                       call prism_sys_abort(compid,subname,' ERROR namcouple field names do not match for in/out')
                    endif
                 endif
                 ...
                 if (flag == PRISM_In .and. found) then
                    ...
                    call prism_sys_abort(compid,subname,' ERROR found two source '//trim(otfld))
                 endif
                 found = .true.
                 ...
                 if (prism_ncoupler > prism_mcoupler) then
                    write(nulprt,*) subname,' ERROR prism_ncoupler to large',prism_ncoupler,prism_mcoupler
                    call prism_sys_abort(compid,subname,' ERROR prism_ncoupler too large')
                 endif
                 nc = prism_ncoupler
                 ...
                 if (namfldops(nn) == ip_exported .or. namfldops(nn) == ip_expout) then
                    ...
                    if (prism_nrouter > prism_mrouter) then
                       write(nulprt,*) subname,' ERROR prism_nrouter to large',prism_nrouter,prism_mrouter
                       call prism_sys_abort(compid,subname,' ERROR prism_nrouter too large')
                    endif
...
!        call prism_coupler_print(nc)
  ...
  do nc = 1,prism_ncoupler
     ...
     if (part1 <= 0) then
        ...
        call prism_sys_abort(compid,subname,' ERROR part1 invalid')
     endif
...
!-------------------------------------------------------------------------------
 ...
 if (mytask == 0) then
   ...
   if (status /= NF_NOERR) then
      ...
      call prism_sys_abort(compid,subname,'ERROR filename')
   endif
...
!      end if
   ...
   elseif (newdom == 'dst') then
      ...
      call prism_sys_abort(compid,subname,' invalid newdom value')
   endif
   lsize = 0
   ...
   do n = 1,lsize-1
      if (lsstart(n) > lsstart(n+1)) then
         ...
         call prism_sys_abort(compid,subname,'lsstart not sorted')
      endif
   enddo
