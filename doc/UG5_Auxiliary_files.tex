...
\item {\em grids.nc}: contains the model grid longitudes and latitudes
  ...
  available in the {\em grids.nc} file as double precision {\tt
    REAL} arrays dimensioned {\tt (nx,ny,nc)} or {\tt (nbr\_pts,1,nc)}
  where {\tt nc} is the number of corners (in the counterclockwize
  sense, starting by any corner) that can by any number. 
