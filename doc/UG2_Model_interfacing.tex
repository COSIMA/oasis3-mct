...
\begin{itemize} 
\item {\tt CALL prism\_set\_couplcomm(new\_comm, kinfo)}
\item {\tt CALL oasis\_set\_couplcomm(new\_comm, kinfo)}
\begin{itemize}
...
prism\_create\_couplcomm can be used instead of this interface to
achieve the same feature.

This should be called early in the initialization, after the oasis\_init\_comp
...
\begin{verbatim}
  ...
  CALL oasis_set_couplcomm(couplingComm, ierror)
...
In particular, most of the oasis initialization calls are collective
...
other rules that must be followed.  First, oasis\_set\_couplcomm
...
the root of the coupling communicator.  Third, the partition
must be defined only across the coupling processes even though
the def\_partition must be called on all processes.  Finally, the
def\_var must be called by at least the root process of the coupler
communicator but can be called from all processes.

\begin{itemize} 
\item {\tt CALL prism\_create\_couplcomm(icpl, all\_comm, new\_comm, kinfo)}
\item {\tt CALL oasis\_create\_couplcomm(icpl, all\_comm, new\_comm, kinfo)}
\begin{itemize}
\item {\tt icpl [INTEGER; IN]}: couple process flag
\item {\tt all\_comm [INTEGER; IN]}: mpi intercomm communicator
\item {\tt new\_comm [INTEGER; OUT]}: mpi intercomm communicator
\item {\tt kinfo [INTEGER; OUT; OPTIONAL]}: returned error code
\end{itemize}
\end{itemize}

This routine is similar to prism\_set\_couplcomm.  It allows a user
to reset the coupling communicator to a subset of processes.  all\_comm
is the mpi communicator associated with all processes on the component
and icpl is an integer flag associated with each process.  icpl should be
1 on coupling processes and MPI\_UNDEFINED on other processes.  The new
coupler communicator will be returned in the new\_comm argument and
the internal coupling communicator will also be set to that value.  See
prism\_set\_couplcomm for more information about why and how this 
