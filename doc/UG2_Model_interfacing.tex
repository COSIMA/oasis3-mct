...
\item {\tt CALL oasis\_terminate\_grids\_writing()}
  ...
  Obsolete in OASIS3-MCT; can be called for upward compatibility.
...
\item {\tt CALL prism\_def\_var\_proto(var\_id, name, il\_part\_id,
  ...
  \begin{itemize}
  \item {\tt var\_id [INTEGER; OUT]}: coupling field ID. Note that if
      the field does not appear in the {\it namcouple}, {\tt var\_id} will be equal to -1 and it is not permitted to call the corresponding {\tt oasis\_put}
      and {\tt oasis\_get} (it would lead to an abort). Conversely, all
      coupling fields appearing in the {\it namcouple} must be defined
      with a call to {\tt oasis\_def\_var}; this constraint is imposed
      to avoid that a typo in the  {\it namcouple}  would lead to
