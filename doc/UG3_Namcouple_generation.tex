...
\item {\tt \$NMAPDEC}: Optional; on the line below this keyword is a character string
  ...
  options are decomp_1d and decomp_wghtfile.  decomp_1d decomposes the grid in a simple
  one dimensional, 1 chunk per pe.  decomp_wghtfile decomposes the grid using the
  information in the weight file to reduced mapping communication.  decomp_wghtfile
  will take some extra time in initialization but it should result in faster mapping.
  The default in decomp_wghtfile.
