...
\item {\tt \$NWGTOPT}: Optional; on the line below this keyword is a character string
  that indicates how to handle bad interpolation weights.  There are four options, 
  ...
  \begin{itemize}
  \item abort\_on\_bad\_index will write error messages to the log files and abort if a bad weight
  index is detected. This is the default option. 
  \item ignore\_bad\_index will write an error message and then remove bad
  weights internally before continuing.  
  \item ignore\_bad\_index\_silently will remove bad weights and continue without writing an error
  message.  
  \item use\_bad\_index will attempt to keep bad weights in the interpolation computation, 
