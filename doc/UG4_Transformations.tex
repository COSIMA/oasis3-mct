...
\end{verbatim}
    ...
    \item {\tt \$VAR} defines the weight given to a neighbour source grid
      ...
      $\overline{D}^2$ is the average distance between two source grid points 
      (calculated automatically but approximatey by OASIS3-MCT).
    \end{itemize}
  ...
  \item {\tt BICUBIC} performs an interpolation based on a local
    ...
    For {\tt BILINEAR} and {\tt BICUBIC}, the configuring line
    is:
...
\end{verbatim}
    ...
    \item Non-masked target grid points having all source
      ...
      neighbour is used ({\tt ll\_nnei} hard-coded to {\tt .true.} in corresponding SCRIP sources).
...
\item The {\tt gather} option computes global sums by gathering a decomposed array
  ...
  both with respect to performance and memory use. This is equivalent to the {\tt bfb} option in previous OASIS versions.
