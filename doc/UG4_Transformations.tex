...
to prior versions will be noted with the string NON\_FUNCTIONAL
but not described.
...
Transformation {\tt LOCTRANS}\footnote{Note that LOCTRANS cannot be
  chosen with the deprecated {\tt SIPC}, {\tt PIPE}, or {\tt GMEM} communication
  techniques.} requires one configuring line on which a time
...
filenames are now required for all namcouple transforms that use
...
\item {\bf REDGLO} NON\_FUNCTIONAL
...
\item {\bf INVERT}: NON\_FUNCTIONAL
...
\item {\bf EXTRAP}: NON\_FUNCTIONAL
...
% field, and {\tt \$NBFIELDS} is NON\_FUNCTIONAL and must be set to 1.
% \begin{verbatim}
% # nbfields lines
...
% are all NON\_FUNCTIONAL but still required for parsing.  
...
# BLASOLD operation
     $XMULT   $NBFIELDS \end{verbatim} where
{\tt \$XMULT} is the multiplicative coefficient of the current
field, and {\tt \$NBFIELDS} is NON\_FUNCTIONAL and should be
set to 0 or 1.  For each additional field, an
...
\begin{itemize}
  \item \$MAPNAME is the name of the mapping file to read.  This is a netcdf
  file consistent with the OASIS mapping format.

  \item \$MAPLOC is either src or dst.  With src, the mapping will be done
  in parallel on the source processors before communication on the destination
  grid to the destination model and processors.  With dst, the mapping is 
  done on the destination processors after the data is sent from the source
  model on the source grid.  src is default.  This field is optional.

  \item \$MAPSTRATEGY is either bfb, sum, or opt.  In bfb mode, the mapping is
  done using a strategy that produces bit-for-bit identical results regardless
  of the grid decompositions with leveraging a partial sum computation.  With
  sum, the transform is done using the partial sum approach which generally
  introduces roundoff level changes in the results on different processor
  counts.  opt allows the couplng layer to choose either approach based on
  an analysis of which strategy is likely to run faster.

\end{itemize}

\item {\bf SCRIPR}: NON\_FUNCTIONAL for now
  ...
  \item {\tt DISTWGT} performs a distance weighted nearest-neighbour
    interpolation (N neighbours). All types of grids are supported. 
     ...
     \item Non-masked target grid points having all of the N source
       ...
       changed since release {\tt oasis3\_prism\_2\_5}\footnote{To reproduce the previous default
         behaviour, one has to compile with CPP key {\tt
           NOT\_NNEIGHBOUR}. In this case, the zero value is
         associated to the target points having all of the N source
         nearest neighbours masked. However, the value 1.0E+20 will be
         assigned to these non-masked target grid points if routines
         {\tt scriprmp.f} or {\tt vector.F90} (for vector
         interpolation) in {\tt oasis3/lib/scrip/src/} are compiled
         with {\tt ll\_weightot=.true.}.}.
 ...
 # SCRIPR/DISWGT 
  ...
  below). The option {\tt VECTOR}, which is fact leads to a scalar treatment
...
% XXXX modifier la routine et la doc XXXXX 
 ...
 $CMETH  $CGRS  $CFTYP  $REST  $NBIN  $NV $VAR $ASSCMP $PROJCART 
  ...
  \item {\tt BICUBIC} performs an interpolation based on a local bicubic approximation
    (see details
...
\vspace{1cm}
\item {\bf INTERP}: NON\_FUNCTIONAL
...
\item {\bf MOZAIC}: NON\_FUNCTIONAL
...
\item {\bf FILLING}: NON\_FUNCTIONAL
...
\item {\bf CONSERV}: 
  ...
  {\tt CONSERV} (routine {\tt oasis3/src/conserv.f}) ensures
  a global modification of the coupling field. This `cooking'' stage operation 
  should not be mixed with  {\tt SCRIPR/CONSERV} conservative remapping (see section \ref{subsec_interp}). 

  This analysis requires one input line with one argument:

 \begin{verbatim}
# CONSERV operation
     $CMETH\end{verbatim}  where {\tt \$CMETH} is the method required:
...
\item {\bf SUBGRID}: NON\_FUNCTIONAL
...
{\tt BLASNEW} (routine {\tt oasis3/src/blasnew.f}) performs a scalar
...
OASIS3, controlled by {\tt oasis3/src/postpro.f}.

\begin{itemize}

\item {\bf REVERSE}: NON\_FUNCTIONAL
...
\item {\bf GLORED}: NON\_FUNCTIONAL
