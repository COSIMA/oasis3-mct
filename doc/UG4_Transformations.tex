...
transformation (with {\tt CLIM/MPI1-MPI2} communication technique only), pre-processing, 
...
\item {\bf CHECKIN}:
 ...
 {\em cplout} (this operation does not transform the field).
...
The following transformations, controlled by {\tt interp.f}, are
available in OASIS3.
...
$CMETH  $CGRS  $CFTYP  $REST  $NBIN  $NV $VAR $ASSCMP $PROJCART\end{verbatim} where all entries are as for  {\tt DISTWGT}, except that:
  ...
  \item Non-masked target grid points having all of the source points
  normally used in the bilinear or bicubic interpolation masked: the
  nearest non-masked source neighbour is used.
  \end{itemize} 
...
# CONSERV operation
     $CMETH\end{verbatim}
   where {\tt \$CMETH} is the method required:
   \begin{itemize}
...
{\tt BLASNEW} (routine {\tt oasis3/src/blasnew.f}) performs a linear combination of the
...
{\tt CHECKOUT} (routine {\tt oasis3/src/chkfld.f}) calculates
the mean and extremum values of an output field and prints them to the
coupler output {\em cplout} (this operation does not transform the field).
