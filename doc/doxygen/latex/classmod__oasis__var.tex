...
\begin{DoxyItemize}
\item Check whether all tasks have same vars defined. If so, then skip a bunch of the work to sort it out.
\item Gather variable information across all tasks
\begin{DoxyItemize}
\item Gather number of variables
\item If gatherall, Gather variable names
\item If gatherall, Gather partition names
\item If gatherall, Gather in/out variable setting
\end{DoxyItemize}
\item If gatherall, Determine the unique variable names on the root
\item If not gatherall, Loop over each task
\begin{DoxyItemize}
\item If not gatherall, Send variable, partition, inout to root
\item If not gatherall, Recv variable, partition, inout on root
\item If not gatherall, Determine the unique variable names on the root
\end{DoxyItemize}
\item Broadcast the variable information to all tasks
