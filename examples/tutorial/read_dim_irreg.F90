!****************************************************************************************
SUBROUTINE read_dim_irreg (nlon,nlat,corners_ij_lus,data_filename,w_unit)
  !**************************************************************************************
  USE netcdf
  IMPLICIT NONE
  !
  ...
  IF ( (lat_dims_len(1) .NE. lon_dims_len(1)).OR.(lat_dims_len(2) .NE. lon_dims_len(2)) ) THEN
      WRITE(w_unit,*) 'Problem model1 in read_dim_irreg'
  ...
  ENDDO
  !
  IF ( (clo_dims_len(1) .NE. lon_dims_len(1)) .OR. (clo_dims_len(2) .NE. lon_dims_len(2))) THEN
      WRITE(w_unit,*) 'Problem model1 in read_dim_irreg'
      WRITE(w_unit,*) 'Dimensions of the longitude of the corners are not the same as the ones of the longitude'
      CALL flush(w_unit)
      STOP
  ENDIF
  !
  corners_ij_lus=clo_dims_len(3)
  !++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
  !
  ...
  ENDDO
  !
  IF ( (cla_dims_len(1) .NE. lon_dims_len(1)) .OR. (cla_dims_len(2) .NE. lon_dims_len(2))) THEN
      WRITE(w_unit,*) 'Problem model1 in read_dim_irreg'
      WRITE(w_unit,*) 'Dimensions of the latitude of the corners are not the same as the ones of the longitude'
      CALL flush(w_unit)
      STOP
  ENDIF
  !
  IF (cla_dims_len(3) .NE. clo_dims_len(3)) THEN
      WRITE(w_unit,*) 'Problem model1 in read_dim_irreg'
      WRITE(w_unit,*) 'Problem of the dimension of the corners in the (i,j) plan'
      WRITE(w_unit,*) 'Number of corners for the longitude :',clo_dims_len(3)
      WRITE(w_unit,*) 'Number of corners for the latitude :',cla_dims_len(3)
      CALL flush(w_unit)
      STOP
  ENDIF
  !++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
