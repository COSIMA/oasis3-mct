PROGRAM atmos
  ...
  comp_out='atmos_component.out_'//chout
  !
  OPEN(w_unit,file=TRIM(comp_out),form='formatted')
  WRITE (w_unit,*) '-----------------------------------------------------------'
  WRITE (w_unit,*) 'MPI_COMM_WORLD is :',MPI_COMM_WORLD
  WRITE (w_unit,*) 'In MPI_COMM_WORLD, I am atmos_component process with rank :',mype
  WRITE (w_unit,*) '----------------------------------------------------------'
  CALL flush(w_unit)
  ! 
  ! OA 
  ...
  WRITE(w_unit,*) 'In my local communicator I am atmos_component process with local rank', mype
  ...
  !OA
  !!!!!!!!!!!!!!!!! OASIS_WRITE_GRID  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ! The chosen grid acronym is 'lmdz'
  !
  CALL oasis_start_grids_writing(il_flag)
  CALL oasis_write_grid('lmdz', nlon, nlat, grid_lon, grid_lat)
  CALL oasis_write_corner('lmdz', nlon, nlat, 4, grid_clo, grid_cla)
  CALL oasis_write_area('lmdz', nlon, nlat, grid_srf)
  CALL oasis_write_mask('lmdz', nlon, nlat, grid_msk(:,:))
